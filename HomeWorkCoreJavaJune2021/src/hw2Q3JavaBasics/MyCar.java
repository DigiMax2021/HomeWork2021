  
package lec03_java_basics_06272021;

public class B_use_of_newline_tab_backspace_carriagereturn {

	public static void main(String[] args) {
		System.out.println("In 1885, a 22-year-old Dutch woman named Johanna Bonger met Theo van Gogh, the younger brother of the artist. He asked her to marry him after only two meetings. In 1888, a year and a half after his proposal, she agreed. It was Paris in the belle epoque: art, theater, intellectuals, the streets of their Pigalle neighborhood raucous with cafes and brothels.");
		
		System.out.println("****************************************************************************");
		System.out.println("Hi");
		
		// use of \n --> new line  ---> a lot of use of \n
		// use of \t --> tab  ---> a lot of use of \t
		// use of \b --> back space 
		// use of \r --> carriage return 
		
		System.out.println("\n\t In 1885, a 22-year-old Dutch woman named Johanna Bonger met Theo van Gogh, the younger brother of the artist. He asked her to marry him after only two meetings. \n In 1888, a year and a half after his proposal, she agreed. \n\t It was Paris in the belle epoque: art, theater, intellectuals, the streets of their Pigalle neighborhood raucous with cafes and brothels.");

	}
}